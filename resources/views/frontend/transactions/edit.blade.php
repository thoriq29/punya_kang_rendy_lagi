        ...
        <div class="col-md-12 mb-3">
          <div class="alert alert-light alert-dismissible show fade">
            <div class="alert-body">
              ...
              <i class="fa fa-info"></i> &nbsp <strong>Untuk pembayaran cicilan silahkan membayar Uang Muka sebesar minimal Rp 10.000.000</strong>
            </div>
          </div>
        </div>
      @endif
              ...
              <div class="card-body">
                @if ($order->payment_type == "parsial")  
                  <div>
                    ...
                    <strong><span class="badge badge-light"><strong>{{ ($order->payment->count() == 0) ? "Down Payment (DP)" : "Pembayaran Ke-" . $order->payment->count() }}</strong></span></strong>
                  </div>
                  <hr>
                @endif
                <form id="frm-payment" action="{{ route('transaction.update', $order->id) }}" method="post" enctype="multipart/form-data">
                  @csrf
                  <div class="form-group {{ $errors->has('method') ? ' has-error' : '' }}">
                    <label for="method" class="d-block">Metode pembayaran</label>
                    <select name="method" class="form-control select2 @if ($errors->has('method')) is-invalid @endif">
                  ...
                  <div class="form-group {{ $errors->has('account_holder') ? ' has-error' : '' }}">
                    <label for="account_holder" class="d-block">Transafer Atasa Nama</label>
                    <input type="text" class="form-control @if ($errors->has('account_holder')) is-invalid @endif" name="account_holder" value="{{ $order->account_holder }}">
                  ...
                  @php
                      $pay = (int)$order->price_total;
                      if ($order->payment_type == "parsial"){
                        if ($order->payment->count() < 1) {
                          $pay = 10000000;
                        } else {
                          $pay = $paid_data["paid_remain"];
                        }
                      }
                  @endphp

                  <div class="form-group {{ $errors->has('paid') ? ' has-error' : '' }}">
                    <label for="paid" class="d-block">Total Transfer</label>
                    <input type="text" class="form-control @if ($errors->has('paid')) is-invalid @endif" name="paid" value="{{ $pay }}">
                    @if ($errors->has('paid'))
                      <div class="invalid-feedback">
